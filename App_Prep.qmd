...
df_corr <- mobile_usage |>
  ...
  rename("App Usage" = app_usage_time_min_day,
...
ggcorr(df_corr, 
       ...
       hjust = 0.63) +    # adjusting position of variable names
...
Plot 6: Cross between Scatterplot and 2D Density Plot

```{r}
library(ggpointdensity)
library(viridis)

g + geom_pointdensity(aes(x = age, y = screen_on_time_hours_day)) +
  scale_color_viridis() +
  labs(x = "Age", 
       y = "Screen Time (hours per day)",
       title = "Screen Time by Age",
       subtitle = "by Operating System") +
...
```{r}
g + geom_density(aes(x = data_usage_mb_day, fill = operating_systemF), 
                 alpha = 0.3) +
  scale_y_continuous(expand = expansion(mult = c(0, 0.05))) +
  labs(x = "Data Usage (mb/day)", 
       y = "Density",
       title = "Distribution of Data Usage by Operating System") +
  theme(plot.title = element_text(hjust = 0.5))
```
